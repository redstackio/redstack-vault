...
- **Payload Generation**: Create single-stage or staged payloads for execution, shells, and Meterpreter sessions.
- **Encoding and Encryption**: Apply multiple encoding layers (e.g., x86/shikata_ga_nai) and encryption options to bypass antivirus.
- **Format Flexibility**: Output in formats like EXE, DLL, ELF, APK, raw hex, or scripting languages (Python, Ruby, PowerShell).
- **Template Support**: Embed payloads into custom templates for stealthy delivery.
- **Integration**: Works seamlessly with Metasploit's exploit modules and can generate payloads for use with handlers like multi/handler.
